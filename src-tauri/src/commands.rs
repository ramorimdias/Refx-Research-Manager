...
CREATE INDEX IF NOT EXISTS idx_graph_view_node_layouts_graph_view_id ON graph_view_node_layouts(graph_view_id);
    ...
    ensure_column(&conn, "references", "chapter", "TEXT")?;
    ensure_column(&conn, "references", "is_manual", "INTEGER NOT NULL DEFAULT 0")?;
...
fn map_work_reference_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<WorkReference> {
    Ok(WorkReference {
        ...
        reference: Reference {
            ...
            r#type: row.get(11)?,
            is_manual: row.get::<_, i64>(12)? == 1,
            citation_key: row.get(13)?,
            title: row.get(14)?,
            authors: row.get(15)?,
            year: row.get(16)?,
            journal: row.get(17)?,
            ...
            chapter: row.get(20)?,
            pages: row.get(21)?,
            publisher: row.get(22)?,
            booktitle: row.get(23)?,
            doi: row.get(24)?,
            url: row.get(25)?,
            abstract_text: row.get(26)?,
            keywords: row.get(27)?,
            bibtex: row.get(28)?,
...
fn normalize_reference_type(input: &str) -> String {
    match input.trim() {
        "manual" => "misc".to_string(),
...
pub fn create_reference(
    ...
    let now = now_iso();
    let normalized_type = normalize_reference_type(&input.r#type);
    let is_manual = input.is_manual.unwrap_or(false);
    ...
    conn.execute(
        ...
        params![
            ...
            input.chapter.map(|value| value.trim().to_string()).filter(|value| !value.is_empty()),
...
pub fn update_reference(
    ...
    conn.execute(
        ...
        UPDATE "references" SET
          ...
          is_manual = COALESCE(?3, is_manual),
          citation_key = COALESCE(?4, citation_key),
          title = COALESCE(?5, title),
          authors = COALESCE(?6, authors),
          year = COALESCE(?7, year),
          journal = COALESCE(?8, journal),
          volume = COALESCE(?9, volume),
          issue = COALESCE(?10, issue),
          chapter = COALESCE(?11, chapter),
          pages = COALESCE(?12, pages),
          publisher = COALESCE(?13, publisher),
          booktitle = COALESCE(?14, booktitle),
          doi = COALESCE(?15, doi),
          url = COALESCE(?16, url),
          abstract = COALESCE(?17, abstract),
          keywords = COALESCE(?18, keywords),
          bibtex = COALESCE(?19, bibtex),
          updated_at = ?20
        WHERE id = ?21
        "#,
        params![
            input.document_id,
            input.r#type.map(|value| normalize_reference_type(&value)),
            input.is_manual.map(|value| if value { 1 } else { 0 }),
            input.citation_key.map(|value| value.trim().to_string()).filter(|value| !value.is_empty()),
