...
export function DiscoverWorkRow({ work }: { work: DiscoverWork }) {
  ...
  return (
    <div
      ...
      className={cn(
        'w-full min-w-0 overflow-hidden rounded-2xl border bg-card px-3 py-2.5 transition',
        selectedWorkId === work.id ? 'border-primary/40 bg-primary/5' : 'border-border hover:border-primary/40 hover:bg-accent/30',
        hoveredWorkId === work.id && 'border-sky-400/70 bg-sky-400/10',
      )}
    >
